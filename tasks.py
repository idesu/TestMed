...
#                      if x["userId"] == user["id"] and x["completed"]]
        ...
        uncompleted_tasks = (
            ''.join(
            x["title"]+'\n' if len(x["title"]) < 50 else x["title"][:50]+'...\n'
            for x in tasks
        ...
        if os.path.isfile(f'tasks/{user["username"]}.txt'):
            ...
            formatted_time = dt.datetime.fromtimestamp(creation_time).strftime('%Y-%m-%dT%H:%M')
            os.rename(f'tasks/{user["username"]}.txt', f'tasks/{user["username"]}_{formatted_time}.txt')

        try:
            with open(f'tasks/{user["username"]}.txt', 'w') as f:
                f.write(f'{user["name"]} <{user["email"]}> {time_now.strftime("%d.%m.%Y %H:%M")}\n')
                f.write(f'{user["company"]["name"]}\n\n')
                f.write("Завершённые задачи:\n")
                f.write(completed_tasks)
                f.write("\nОставшиеся задачи:\n")
                f.write(uncompleted_tasks)
